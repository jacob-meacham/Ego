...
bool Pathfinder::FindPath(char* map, long startingX, long startingY, long destX, long destY) {
	...
	while(1) { // run until we find a solution
		...
		for(int index = 1; index<=8; index++) {
			...
			else if(!isWalkable(&test)) { } // if the node is unwalkable, we can ignore it
			
			else if(onClosed(&test)) { } // if the node is on the closed list, we ignore it
			
			else if(!onOpen(&test)) { // if it is not yet on the open list, we may calculate H and G and add it to the list
			...
			else if(onOpen(&test)) { // if it is already on the open list, we see if the current path is better
				std::list<Node>::iterator iNode;
				for (iNode = openList.begin(); iNode != openList.end(); iNode++) {
...
*/
int Pathfinder::CalculateHVal(long posX, long posY, long destX, long destY) {
...
*/
int Pathfinder::CalculateGVal(Node *parent, Node *test) {
	int addedCost = 0;
...
bool Pathfinder::isDestination(Node *current, long destX, long destY) {
...
*/
bool Pathfinder::isWalkable(Node *current) {
...
*/
Node Pathfinder::GetNeighbor(int index, Node *current) {
	Node neighbor;
	switch(index) {
		case 1:
